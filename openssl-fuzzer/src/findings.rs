...
pub fn generate_finding_script(
    ...
    script.push_str("#\n");
    script.push_str("# openssl-fuzzer Finding Report\n");
    script.push_str("#\n");
...
pub fn write_finding(
    ...
    let script_content = generate_finding_script(openssl_command, fitness, exit_code, signal, notes);
...
mod tests {
    ...
    fn test_script_content() {
        let script = generate_finding_script(
            "echo Q | openssl s_client -connect localhost:8443 -tls1_2",
            100.0,
            -1,
            Some(11),
            "SIGSEGV crash",
        ...
        assert!(script.contains("openssl-fuzzer Finding Report"));
        ...
        assert!(script.contains("echo Q | openssl s_client"));
    }
    ...
    fn test_write_finding_creates_file() {
        let temp_dir = std::env::temp_dir().join("openssl-fuzzer-test-findings");
