...
impl FuzzDatabase {
    ...
    pub fn insert_run(&self, run: &RunRecord) -> Result<()> {
        self.conn.execute(
            "INSERT INTO runs (
                generation, individual_id, ls_command, fitness_total,
                fitness_components, exit_code, signal, duration_ms, stderr
    ...
    pub fn get_top_findings(&self, limit: usize) -> Result<Vec<RunRecord>> {
        let mut stmt = self.conn.prepare(
            "SELECT generation, individual_id, ls_command, fitness_total,
                    fitness_components, exit_code, signal, duration_ms, stderr,
                    created_at
             FROM runs
...
mod tests {
    ...
    fn test_insert_and_query() {
        ...
        let run = RunRecord {
            generation: 1,
            individual_id: 0,
            ls_command: "ls -la /tmp".to_string(),
            fitness_total: 105.0,
        ...
        assert_eq!(results[0].ls_command, "ls -la /tmp");
    }
    ...
    fn test_generation_stats() {
        ...
        for i in 0..5 {
            let run = RunRecord {
                generation: 1,
                individual_id: i,
                ls_command: format!("ls command {}", i),
